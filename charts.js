...
class Chart {
    ...
    draw() {
        ...
        switch (this.options.type) {
            ...
            case 'area':
                ...
                break;
            case 'horizontalBar':
                this.drawHorizontalBarChart(width, height);
                break;
            case 'radar':
    ...
    drawHorizontalBarChart(width, height) {
        const { data, labels, colors } = this.options;
        const isSmallScreen = width < 400;
        const paddingLeft = isSmallScreen ? 60 : 80;
        ...
        // Avoid drawing if dimensions are invalid
        if (chartWidth <= 0 || chartHeight <= 0) return;

        data.forEach((value, index) => {
            const y = paddingTop + (barHeight + gap) * index;
            const targetWidth = (value / maxVal) * chartWidth;
            const barWidth = targetWidth * this.progress;

            // Bar
            const gradient = this.ctx.createLinearGradient(paddingLeft, 0, paddingLeft + barWidth, 0);
            gradient.addColorStop(0, colors[index % colors.length]);
            gradient.addColorStop(1, this.darkenColor(colors[index % colors.length], 0.2));

            this.ctx.fillStyle = gradient;
            this.ctx.fillRect(paddingLeft, y, barWidth, barHeight);
            ...
            if (this.progress > 0.8) {
                ...
                this.ctx.fillText(value, paddingLeft + barWidth + 5, y + barHeight / 2);
    ...
    drawRadarChart(width, height) {
        ...
        data.forEach((value, i) => {
            const angle = i * stepAngle - Math.PI / 2;
            const r = (value / maxVal) * radius * this.progress;
            const x = cx + Math.cos(angle) * r;
            const y = cy + Math.sin(angle) * r;
            if (i === 0) this.ctx.moveTo(x, y);
            else this.ctx.lineTo(x, y);
        ...
        this.ctx.closePath();
        this.ctx.fillStyle = colors[0] + '40'; // Transparent fill
        this.ctx.fill();
